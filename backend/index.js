...
app.post("/login", async (req, res) => {
  const token = jwt.sign({ userId: "asldkhsakldjasl" }, config.app_secret);
...
app.get("/places", async (req, res) => {
  const places = await Place.find().populate("category").exec();
...
app.post("/upload-image", function (req, res, next) {
  return res.json({
    main_image: [...req.files.map((file) => file.originalname)],
  });
});
...
app.post("/categories", Authentication, async (req, res) => {
  const { name } = req.body;
...
app.put("/category", async (req, res) => {
  ...
  const updatedCategory = await (
    await Category.findByIdAndUpdate({ _id: category._id }, category)
  ).exec();
  return res.json({
